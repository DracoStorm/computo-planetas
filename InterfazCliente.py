...
def enviar_archivo_interfaz():
    ...
    progreso_label = tk.Label(progreso_window, background='#31363F', fg='white', font=('Arial', 20))
    ...
    progreso_label.config(text=f"Archivo enviado con éxito al servidor:\n{archivos_enviados[-1]}")

    # Mostrar todos los archivos enviados durante la conexión
    archivos_label.config(text="Archivos enviados:\n" + "\n".join([f"{i + 1}. {archivo}" for i, archivo in enumerate(archivos_enviados)]))
...
archivos_label = tk.Label(root, background='#31363F', fg='white', font=('Arial', 16))
